      ...
      {selectedTab === 'info' && (
        <ScrollView style={styles.infoContainer}>
          ...
          <Text style={styles.description}>
            {resource.descripcion?.ops?.map(op => op.insert).join('') ?? resource.descripcion ?? ''}
          </Text>
          <Text style={styles.sectionTitle}>Condiciones de uso</Text>
          <Text style={styles.description}>
            {resource.condiciones_uso?.ops?.map(op => op.insert).join('') ?? resource.condiciones_uso ?? ''}
          </Text>
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  resourceName: { fontSize: 24, fontWeight: 'bold', textAlign: 'center', color: '#1E293B' },
